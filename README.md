...
`mapground` 提供可搜索的地面节点，`mapcloud` 提供静态三维障碍，实时感知再提供观测点和致命障碍点。dGraph 的红点表示障碍距离较小或已致命的地面位置，不等同于原始激光回波。

当前 `odin1_navigation.yaml` 没有覆盖 `use_pre_graph`，因此采用代码默认值 `false`，实际调用 `A_Star_on_Graph::getPath()`：每次扩展节点时在地面点云 KD-tree 中做半径搜索，动态取得候选邻居。只有显式把 `use_pre_graph` 设为 `true`，才会走预先建立连边的 `A_Star_on_PreGraph` 分支。

#### 18.2.1 一次规划请求的完整调用顺序

```text
Web 路径预览                         正式导航
/api/preview-navigation-goal        /api/start-navigation-goal
       │                                    │
       │ 直接请求 /get_plan                 │ 检查连接、控制权、定位并启用运动
       │ 不发布运动目标                     │ 发布 /goal_pose_3d
       ▼                                    ▼
GlobalPlanner::makePlan()           p2p_move_base / global_plan_manager
       │                                    │ 请求 /get_dwa_plan 并持续管理任务
       │                                    ▼
       └──────────────────────► Dynamic-Window-Aware 全局规划包装层
                                      │ 内部请求基础全局路径
                                      ▼
                               GlobalPlanner::makePlan()
                                      │
                                      ▼
                               GlobalPlanner::makeROSPlan()
                                      │
                         ┌────────────┼────────────┐
                         ▼            ▼            ▼
                  getStartGoalID()  getPath()   getROSPath()
                    匹配图节点       A* 搜索     转为 ROS Path
                                                   │
                                                   ▼
                                         补起点、终点直连段
                                                   │
...
`makePlan()` 先检查静态感知层是否就绪，并检查请求中的 `activate_threading`。该字段为 `true` 时只触发本次计算，不会在这个函数内部创建永久规划线程。起点不由网页传入，而是由感知模块读取当前全局机器人位姿；随后调用 `makeROSPlan()`。返回路径为空时 Action 以失败结束，非空时发布基础 `global_path` 并把同一路径放入 Action 结果。路径预览只走到这里，不获取控制权，也不发送速度。

正式导航中，任务管理器使用 `/get_dwa_plan` **action** 获取动态感知路径，控制器跟踪 `/awared_global_path`。动态包装层先取得完整路线，再依据当前位置、前视窗口和动态感知更新相关路径。名字中的 DWA 不表示基础 A* 只做速度采样。模板的 `look_ahead_distance=2.0 m`、`recompute_frequency=10 Hz` 与任务管理器 `global_plan_query_frequency=5 Hz` 属于不同循环。

#### 18.2.2 地面节点是什么

地面节点来自保存地图中的 `mapground`，不是规则网格的几何中心。当前点云先在投影侧经过体素降采样，地图服务器合并关键帧地面后又按 `complete_map_voxel_size` 降采样；模板值为 `0.2 m`，WebUI 生成的运行配置可以覆盖它。PCL `VoxelGrid` 通常输出每个有点体素内输入点的质心，所以节点坐标受体素内实际点分布影响，并不一定落在体素正中心。

墙面投影到地面的实验补点启用时，普通地面点的 `intensity` 为 `0`，推断得到的墙根地面点设为 `1.0`。两级地面体素滤波均保留全部字段，因此同一体素混合普通地面和投影点时，输出 `intensity` 是其平均值。这个字段在这里是规划代价标记，不再表示激光反射强度。

#### 18.2.3 `getStartGoalID()`：把连续坐标匹配为地面节点

目标点和机器人位姿都是连续坐标，A* 使用的却是 `pcl_ground_` 数组索引，因此搜索前必须得到 `start_id` 和 `goal_id`。

目标匹配过程为：

1. 以用户目标 XYZ 为球心，在地面 KD-tree 中搜索半径 `0.5 m` 的三维球。
2. 搜索到点后，当前代码取搜索结果的第一个索引作为 `goal_id`。
3. 首次没有结果时，如果地面图已就绪，则保持 XY 不变，从目标 Z 开始每次向下 `0.1 m`，每层以 `0.3 m` 半径继续搜索，最低搜索到 `-10 m` 以上。
4. 所有搜索都失败，或地面图未就绪时，本次规划失败。

起点匹配需要处理“机身原点在空中、地面节点在脚下”的高度差。当前模板为：

| 参数 | 当前模板值 | 作用 |
|---|---:|---|
| `find_start_tolerance` | `0.5 m` | 起点允许的最大水平距离 |
| `start_ground_offset` | `0.50 m` | 从机身原点 Z 向下得到预期地面 Z |
| `start_ground_z_tolerance` | `0.20 m` | 地面节点相对预期地面高度的容差 |

启用 `start_ground_offset` 后，查询点为 `(start.x, start.y, start.z - 0.50)`。代码先调用：
...
`std::hypot(0.5, 0.2) ≈ 0.5385 m`。这个球形半径只负责一次收集所有可能候选，随后代码分别检查：

```text
水平距离 sqrt(dx² + dy²) <= 0.5 m
高度距离 abs(dz)          <= 0.2 m
```

最后优先选择水平距离最小的节点；水平距离相同时再选择高度差更小的节点。这样可减少机器人被吸附到楼上、楼下或其他高度层地面的风险。若 `start_ground_offset <= 0`，则退回旧逻辑：直接以机身 XYZ 为中心做 `find_start_tolerance` 的三维球形搜索。

`getStartGoalID()` 返回成功只说明两端附近各找到了一个地面节点。两个节点可能分属不连通区域，也可能所有中间候选边都被碰撞检查淘汰，所以此时还不能说“起终点可达”。只有后续 `getPath()` 返回非空索引序列，才表示当前地面图和本轮障碍快照下，匹配后的 `start_id → goal_id` 搜索成功。

#### 18.2.4 `getPath()`：A* 怎样产生候选边

当前 `use_pre_graph=false`，地面点之间没有预先固定的边。A* 每取出一个待扩展节点，就执行：
...
模板中的 `a_star_expanding_radius=0.5 m`，因此半径内每个地面点都是一条“当前节点 → 候选节点”的候选边。`pointIdxRadiusSearch` 保存候选点在地面点云中的索引，`pointRadiusSquaredDistance` 保存当前点到候选点的三维平方距离，开方后作为边长。

候选边只是准备尝试的连线，不等于最终路径，也不等同于局部规划器生成的速度轨迹。它先经过车体扫掠碰撞检查，再计算代价；通过的候选才可能进入 A* 的 open 集合。

A* 的集合状态为：

- **open**：已经发现但还没完成扩展的节点，按最小 `f` 选择下一节点。
- **closed**：已经完成邻居检查的节点，本实现不会重新打开它。
- **parent**：记录以当前最低累计代价从哪个节点到达，用于命中目标后反向回溯。

目标节点进入 closed 后，从目标沿 `parent_index` 回溯到起点，再反转为正向索引序列。如果 open 集合耗尽仍没有关闭目标，`path` 保持为空，表示匹配后的两个节点在本轮搜索中不可达。

#### 18.2.5 候选边的硬碰撞检查

进入 A* 主循环前，`aggregateLethal()` 会汇总各感知插件给出的致命障碍点，`aggregateObservations()` 会汇总用于碰撞判断的观测点。本轮搜索把两份点云复制为固定快照，并分别建立 KD-tree。`kdtree_observation_` 就是实时 observation 点云的空间索引，用来快速查询某个车体包围盒附近有哪些观测点；它本身不生成障碍，也不决定路径代价。

每条候选边调用 `isFootprintSweepClear()`，分别检查三类障碍数据：

| 障碍来源 | 检查方式 |
|---|---|
| 保存地图 `mapcloud` | 完整三维长方体，检查 XY 和高度 |
| 当前实时 observation 点 | 完整三维长方体，检查 XY 和高度 |
| 聚合后的 lethal 点 | 只检查 XY 占地投影，不限制高度 |

当前全局规划车体外形为前后各 `0.47 m`、左右各 `0.278 m`。路径中心位于地面，静态/实时三维检查忽略地面上方 `0.10 m` 以下的点，并检查 `0.10～0.66 m` 的机身高度。扫掠过程按候选边的 XY 方向确定 yaw，沿线段插值 XYZ，每个姿态间隔不超过 `sample_step=0.05 m`；任一采样姿态的长方体内存在障碍点，整条候选边立即淘汰。

KD-tree 的球形搜索只负责快速找出长方体附近的障碍候选，最终是否碰撞仍按车体坐标系中的前、后、左、右、下、上六个边界判断。因此 `inscribed_radius` 已不是 A* 的硬碰撞外形，它继续用于下面的软膨胀代价。

当前实现还有一个几何边界：若候选边 XY 长度小于等于 `1e-6`，扫掠函数直接返回通过，不额外检查纯竖直边的起点和终点。正常地面图通常不会依赖纯竖直连边，但排查多层地面时需要留意。

#### 18.2.6 A* 代价、软膨胀与转弯惩罚
...
new_g = current.g
      + 当前节点到候选节点的三维距离
      + exp(-inflation_descending_rate × (dGraphValue - inscribed_radius))
      + 静态图 node_weight
      + 转角 theta × turning_weight
      + 当前节点邻域的平均 intensity

new_h = 候选节点到目标节点的三维直线距离
new_f = new_g + new_h
```

- **软膨胀**不会直接禁止通行，而是让靠近障碍的节点更贵。当前 `inscribed_radius=0.45 m`、`inflation_descending_rate=2.0`；`dGraphValue` 越小，指数惩罚越大。
- **转弯惩罚**使用“父节点 → 当前节点 → 候选节点”的方向变化 `theta`。当前 `turning_weight=0.1`，增大后 A* 更偏好少转弯的路线，但可能走得更远。
- **node_weight** 来自静态图的节点权重，可反映地面边缘等静态代价。
- **平均 intensity** 是当前节点半径邻域内所有地面点 `intensity` 的均值，然后统一加入从当前节点扩展出去的候选边。普通地面全为 `0` 时此项无影响；邻域包含墙面投影补点时此项大于 `0`，使 A* 更不愿贴近这类区域，但不会把它当作硬障碍。

因此“软膨胀”和“硬碰撞”含义不同：硬碰撞失败会直接 `continue` 淘汰候选边，软代价只影响多条可行路线之间的排序。

#### 18.2.7 `getROSPath()`：索引路径怎样变成航迹

`getPath()` 输出的是地面点索引，`getROSPath()` 将每个索引对应的 XYZ 写入 `nav_msgs/msg/Path`，并为路径点计算方向：

- 非末节点朝向下一个图节点。
- 末节点没有 next，使用“前一点 → 末点”的方向向前外推一个虚拟参考点，避免错误地朝向地图原点。
- 单节点路径没有可用方向，参考点与自身重合。
- `vz != 0` 时走三维分支，把车体 `+X` 轴对齐三维线段方向，姿态可能含 pitch。
- `vz == 0` 时走二维分支，只用 `atan2(vy, vx)` 计算 yaw，roll/pitch 为零。

这里使用精确的 `vz != 0` 判断，极小的地面高度噪声也可能进入三维分支。

随后对每条图节点折线段按 `5%` 比例尝试插值，仅当新点距上一个已输出点大于 `0.1 m` 时才保留。插值只增加同一直线段上的采样点，便于局部规划器裁剪和跟踪；它不会改变 A* 选出的折线几何形状，也不会重新检查或优化路线。

#### 18.2.8 当前起点和终点的直连处理

A* 只连接离散地面节点，实际机身 XY 和用户指定的准确目标通常不恰好落在节点上。当前分支在 `getROSPath()` 之后补两段直线：

```text
实际机身 XY ──直线── A* 第一个地面节点 ──A* 路径── A* 最后一个地面节点 ──直线── 准确目标
```

起点连接段仅在 `start_ground_offset > 0` 且 A* 路径非空时添加：

- 使用实际机身起点的 XY 和首个图节点的 XY 计算水平直线。
- 整段 Z 固定为首个图节点的地面 Z；第一个新增点是“机身当前 XY 对应的地面点”，不是机身原点的三维坐标。
- 复制首个图节点的朝向。
- 按不超过 `0.05 m` 的水平间距插值，并放在原 A* 路径前面。

终点连接段的处理与起点相同思路，但按 XYZ 三维距离插值：

- 从 A* 最后一个地面节点直线连接到请求中的准确目标位置。
- 中间点间距不超过 `0.05 m`，并使用请求目标的朝向。
- 最后原样追加用户请求的准确目标位置和四元数。

**当前这两段直连都不执行车体碰撞检查，也不检查连续地面支撑；准确目标位姿也没有单独做最终车体碰撞检查。** 这是当前分支按“简单连接”的实现选择。它能消除体素地面节点与真实起终点之间的小坐标间隙，但如果匹配容差内存在墙、沟、台阶、未知区或错误地面层，直连段仍可能穿越这些区域。A* 路径非空只证明“首个图节点到最后图节点”的候选边已通过本轮检查，不自动证明这两个额外直连段安全。

分支对照：本次核对的本地 `main` 基线 `501738e` 没有 `start_ground_offset` 起点投影和“机身 XY → 首个路径点”的补段；其 `makeROSPlan()` 在 A* 路径后直接追加准确目标。上面描述的是当前工作分支的实际实现。

#### 18.2.9 规划成功后为什么还可能不走

全局规划成功表示得到了一条非空参考路径，不等于底盘已经收到或能够执行运动：

1. Web 路径预览只调用 `/get_plan`，不获取控制权、不发布 `/goal_pose_3d`。
2. 正式开始导航还要检查 SDK 连接、控制权、定位新鲜度和收敛状态，并启用运动。
3. 局部规划器会基于最新里程计和障碍重新生成短时速度轨迹；全局路径存在时，仍可能因所有局部候选碰撞而停车。
4. `/cmd_vel` 还要经过 Astrall 桥的权限、模式、超时、限幅和轴映射，才会送入设备 SDK。

所以应分别判断“全局路径存在”“局部轨迹可行”“速度已下发”“机身实际响应”，不能用其中一个状态替代其他状态。

#### 18.2.10 从头到尾的数值示例

假设机器人机身位姿为 `(0.00, 0.00, 0.52)`，目标为 `(4.08, 1.02, 0.00)`，地面节点经过体素降采样后大致位于：
...
1. 起点预期地面 Z 为 `0.52 - 0.50 = 0.02 m`。`G0` 的水平距离约 `0.085 m`、高度差 `0.01 m`，同时满足 `0.5 m` 和 `0.2 m` 容差，因此选为 `start_id`。
2. 目标到 `G7` 的三维距离约 `0.082 m`，落在 `0.5 m` 目标搜索半径内，因此选为 `goal_id`。
3. A* 从 `G0` 开始，每次寻找 `0.5 m` 内的邻居。假设 `G1` 到 `G2` 的距离约 `0.50 m`，它可成为候选边；若某条边的车体扫掠长方体命中墙点，该边立即淘汰。
4. 剩余候选按距离、软膨胀、节点权重、转弯和 intensity 累加。若贴墙路线更短，但墙根投影点使邻域平均 `intensity=0.4` 且 dGraph 更小，A* 可能选择稍远的中间路线。
5. 目标 `G7` 进入 closed 后，回溯得到 `G0 → G1 → … → G7`。此时非空 `path` 证明这组图节点在本轮搜索中连通。
6. `getROSPath()` 把节点转成带方向的 Pose，并在较长折线段中增加间隔大于 `0.1 m` 的插值点。
7. 起点直连在地面 Z 上补出 `(0.00, 0.00, 0.01) → G0`；终点直连补出 `G7 → (4.08, 1.02, 0.00)`。这两段只是插值，没有重新做碰撞或支撑检查。
8. 预览模式到此结束并在网页显示路径。正式导航则把参考路径交给局部规划器，局部规划器每周期重新选择可行速度，最后经 `/cmd_vel` 和 Astrall 控制桥驱动机身。

#### 18.2.11 常见失败位置和判断依据

| 现象或日志 | 失败阶段 | 含义 |
|---|---|---|
| `Waiting for static layer` | 静态数据准备 | `mapground`、`mapcloud` 或感知静态层尚未就绪 |
| `Cannot resolve start/goal to ground graph` | 节点匹配 | 起点投影或目标附近没有满足容差的地面节点 |
| `No path found from node ...` | A* 搜索 | 两端节点已匹配，但地面邻域不连通或可连接的候选边被碰撞检查排除 |
| Web 显示“未找到可行路径” | 预览结果检查 | `/get_plan` 失败、返回空路径，或返回点数少于 2 |
| 已显示全局路径但机器人停车 | 局部规划/控制 | 可能没有可行局部轨迹，也可能权限、定位、感知或控制状态不满足 |

排查时先确认当前加载的运行 YAML，而不是只看源码默认值；再按“静态层 → 起终点匹配 → A* 候选边 → ROS 路径 → 局部轨迹 → 控制桥”的顺序定位。相关实现见 [全局规划入口与路径转换](src/dddmr_global_planner/src/global_planner.cpp)、[A* 搜索和碰撞调用](src/dddmr_global_planner/src/a_star_on_pc.cpp)、[长方体扫掠检查](src/dddmr_global_planner/include/global_planner/cuboid_footprint.h) 和 [Astrall 导航模板](../robot/astrall/astrall_dddmr_bringup/config/odin1_navigation.yaml)。
